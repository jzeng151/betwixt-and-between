...
- **Grid shrink can't orphan VISIBLE terrain (codex P2).** `PATCH /api/maps/:id` rejects (409) reducing `grid_cells_x/y` below a cell that is still painted, resolving each cell to its latest biome (a painted-then-erased far cell no longer blocks the shrink). Growth is always allowed.
