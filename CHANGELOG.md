...
### Fixed
- **Same-playhead repaint no longer drops cells (codex P1).** Painting more terrain at the same playhead T after a synthetic anchor was already written there hit the `(world_map_id, t_position)` unique constraint; swallowing the conflict left the stale snapshot shadowing the new same-T events, so the just-painted cells vanished. The auto-anchor now upserts the synthetic anchor's snapshot on conflict (a user-authored anchor at T stays untouched).
...
- **Grid shrink can't orphan painted terrain (codex P2).** `PATCH /api/maps/:id` rejects (409) reducing `grid_cells_x/y` below any existing painted cell; the user must erase the out-of-bounds terrain first. Growth is always allowed.
