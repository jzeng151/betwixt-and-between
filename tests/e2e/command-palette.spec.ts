...
test('refreshes cached notes after edits from entity details', async ({ page, request }) => {
  ...
  await expect(palette.getByRole('option')).toHaveCount(1);
  await search.press('Enter');
  const notebook = page.getByRole('dialog', { name: 'Notes', exact: true });
  await expect(notebook.getByPlaceholder('Entry title')).toHaveValue(renamed);
  await notebook.getByRole('button', { name: 'Back to notes', exact: true }).click();
  await expect(notebook.getByText('Select a folder to view notes')).toBeVisible();
