...
test('fill stroke: drag commits a paint_stroke (mode=fill) and renders without errors', async ({
	...
	// texture load that would otherwise leave the canvas blank-and-settled before
	// the fill paints. (In CI the licensed tile pack is absent; /api/sprites
	// serves a solid placeholder under BETWIXT_E2E_PGLITE so the fill still
	// rasterizes visible pixels — see src/routes/api/sprites/[...path].)
