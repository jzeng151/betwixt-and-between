...
it('exports every saved data table for the authenticated account, including map history and infinity', async () => {
	...
	for (const name of ['Own', 'Foreign']) {
		...
		const [act, character, alias] = await db.insert(schema.entities).values([
			{ userId: user.id, type: 'Act', name: `${name} act`, position: 0 },
			{ userId: user.id, type: 'Character', name: `${name} character`, data: { body: '# Story' } },
			{ userId: user.id, type: 'Character', name: `${name} alias` }
		]).returning();
		await db.insert(schema.relationships).values({ userId: user.id, fromId: character.id, toId: alias.id, type: 'allied_with' });
		await db.insert(schema.entityAliases).values({ primaryEntityId: character.id, aliasEntityId: alias.id });
		await db.insert(schema.intervals).values({ userId: user.id, entityId: character.id, startActId: act.id, endActId: act.id, startPosition: 0, endPosition: 1 });
		await db.insert(schema.canvasPositions).values({ userId: user.id, entityId: character.id });
		await db.insert(schema.windowCanvasState).values({ userId: user.id, windowId: 'graph', entityId: character.id, x: 12, y: 34 });
		const [map] = await db.insert(schema.worldMaps).values({ userId: user.id, name: `${name} map`, baseImageUrl: 'https://example.com/map.png' }).returning();
		...
		await db.insert(schema.mapPlacements).values({ userId: user.id, mapId: map.id, placeableId: character.id, x: 0.2, y: 0.3 });
		await db.insert(schema.factions).values({ userId: user.id, name: `${name} faction`, color: '#123456' });
		await db.insert(schema.worldMapLayerPrefs).values({ userId: user.id, worldMapId: map.id, layerKey: 'terrain', visible: 0 });
	...
	expect(result).toMatchObject({ format: 'betwixt-story', version: 1 });
	expect(Object.keys(result.tables)).toHaveLength(14);
...
it('rejects exports beyond the byte limit', async () => {
	const user = await seedTestUser(db);
	await db.insert(schema.entities).values({ userId: user.id, type: 'Note', name: 'Large', data: { body: 'x'.repeat(17 * 1024 * 1024) } });
	await expect(GET({ locals: { db, user } } as any)).rejects.toMatchObject({ status: 413 });
});
