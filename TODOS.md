...
- [x] **T8b — Better-Auth + multi-user + Cloudflare deploy** — **SHIPPED 2026-05-08** as v0.4.0.0. Deploy target pivoted from Fly.io → Cloudflare Workers (PR #36). Per-request `buildAuth(db, env)` factory with hook-owned db lifecycle (one Pool per request). 22 routes gated by `getUserId(event)`, `intervals.ts` userId-threaded across all 20 public functions. 36 multi-tenant isolation tests across 5 files (entities + cascade-scoping + user FK cascade, relationships, intervals + moveSceneToAct, maps + regions via JOIN, unauthenticated 401). DEPLOY.md covers Neon prod setup, Cloudflare secrets, magic-link Resend gate, rollback. Weekly B2 backup workflow. Vitest: 728 pass / 0 fail. **Known follow-up:** E2E auth bypass code path correct but hits `relation "user" does not exist` specific to PGlite-socket + vite preview + adapter-cloudflare runtime — direct socket scripts work, only playwright fails. Plan: `docs/plans/t8b-cloudflare-rework-20260507.md`. Eng review CLEAR.
