...
- [ ] **T10 — Ghost trails** — faded past/future edges in StoryGraph and FocusedGraph when the scrubber is active. When scrubber sits at T=2.5: edges whose window ended before T render at 0.25 opacity (past), edges whose window starts after T render at 0.15 opacity (future). No schema change needed — uses the existing `start_position`/`end_position` columns from spotlight integration. GraphEdge interface gains optional `ghostMode: 'past' | 'future' | null`. Adds a second pass to the edge render loop in GraphCanvas. Pair with T9 to make ghost opacity a user preference.
