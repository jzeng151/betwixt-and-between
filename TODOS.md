...
- [ ] **Polymorphic `tx | db` refactor** in `src/lib/server/intervals.ts` — `actIndexOf`, `sceneIndexOf`, `assertEntityType`, `assertNoOverlap`, `validateFKTypes`, `computeIntervalPositions`, `recomputeIntervalsForAct`, `recomputeAllIntervals`. All callers thread `tx`. Closes the PR 1 tech-debt note at `intervals.ts:289`. (D17/17A) — **ROLLED INTO T8 (sqlite→Postgres port)** per CONSIDERATIONS.md [2026-04-30]. Drizzle pg supports `db.transaction(async tx => ...)` natively, so the polymorphic-tx work happens as part of the port instead of as a separate refactor. Helpers stay async; `Db` type alias just re-unions over `PgDatabase | PgTransaction`.
...
- [ ] **T8 — sqlite→Postgres port + Auth.js + Neon deploy** (replaces T1) — locked 2026-04-30 in CONSIDERATIONS.md. Scope: schema rewrite (`sqliteTable` → `pgTable`, `text('data')` → `jsonb`, `unixepoch()` → `defaultNow()`), regenerate migrations from scratch in PG dialect, swap `tests/helpers/test-db.ts` from `better-sqlite3 :memory:` to `@electric-sql/pglite` + `drizzle-orm/pglite`, drop the `splitInterval` raw `client.exec('BEGIN/COMMIT')` workaround in favor of `db.transaction(async tx => ...)`, thread polymorphic `Db | Tx` through `intervals.ts` helper chain (rolls in the deferred D17 polymorphic-tx work from line 305), wire Auth.js v5 + `@auth/drizzle-adapter` (4 standard tables: users/accounts/sessions/verificationTokens), set up Neon (free tier or $5/mo warm), configure SvelteKit deploy on Fly.io with `adapter-node`. Local dev via Neon dev branch + `.env.local`; docker-compose fallback for offline. Time estimate ~3-4 focused days with Claude Code. Self-host fallback retained: stay on better-sqlite3 + Tailscale if app stays single-user-forever (~$1-3/mo).
...
**Sequencing note (added 2026-04-30):** T8 (sqlite→Postgres port) should land before Phase 1B, not after. Phase 1B Lane A adds new server endpoints (`/api/canvas-positions/window/[windowId]`, batch upsert) that would otherwise be written in the current sqlite/sync-workaround patterns and have to be retouched during T8. Lane A's "Migration 0002" reference renumbers to "the next migration after T8's regenerated PG baseline" — non-blocking but worth flagging during plan-eng-review of Phase 1B post-T8.
