...
- [ ] **Slice 1a — Projection invariant** (~3 weeks). Decompose `WorldMap.svelte` (1990 LOC) into `src/lib/features/map/` (~12-16 files). New schema (`map_anchors`, `map_events`, `factions`) + migration backfilling initial anchor at `t_position = '-Infinity'::float8` per existing world_map (variants included). `projection.ts` emits typed `RenderedState`; Leaflet stays default renderer; state-based parity tests assert pure-projection equivalence. No new visible behavior. Slice 1 split per 2026-05-21 follow-up eng review (CMT-3): 4-5 week original budget was unrealistic per Codex + subagent cross-model agreement. Burst Step 4.5 stall fallback: if `docs/findings/duplicate-act-on-cross-act-scene-move.md` resolution slips the `intervals.ts` split, Slice 1a's `recomputeMapAnchors` + `recomputeMapEvents` land as a sibling module imported into the still-monolithic `intervals.ts`; file split slips to mid-Slice-1a.
  - **Progress (v0.7.4.0, 2026-05-22):** U2 (schema additions + migration 0012) and U3 (recompute extensions + cascade transaction wrappers + `assertSourceEventIdIsEvent`) shipped. Defense-in-depth scoping on `recomputeMapAnchors`/`recomputeMapEvents` UPDATE paths added per /review. Cross-user JOIN regression test pulled forward from U8. Remaining: U4 (svelte-pixi patch-package + CI hash check), U5 (`projection.ts` pure projector), U6 (`WorldMap.svelte` decomposition into `src/lib/features/map/`), U7 (Slice 1b: Pixi renderer + faction CRUD + transfer_region authoring), U8 (remaining test deltas: D11 migration tests + Vitest invariant test for `assertSourceEventIdIsEvent`).
