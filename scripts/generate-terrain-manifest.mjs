...
// Objects: placeable stamp sprites in static/Sprites/Objects/ (trees, trunks,
// stones, bushes, …). Unlike terrain tiles these carry NO size token in the
// filename, so the `_256_` tile classifier skips them — scan the folder
// directly. Each PNG is one stamp, keyed by its basename (e.g. `tree_object_01`)
// and grouped by family (basename with the trailing `_NN` stripped, e.g.
// `tree_object`) so the palette can offer "trees / bushes / stones" groups.
// Stamp mode of paint_stroke scatters these along a brush path (Slice A).
const objectStamps = [];
const objectsDir = join(SPRITES_DIR, 'Objects');
try {
	for (const f of readdirSync(objectsDir).sort()) {
		if (!f.toLowerCase().endsWith('.png')) continue;
		const key = f.replace(/\.png$/i, '');
		const group = key.replace(/_\d+$/, ''); // tree_object_01 → tree_object
		const label = group
			.replace(/_object$/, '')
			.split('_')
			.map((w) => (w ? w.charAt(0).toUpperCase() + w.slice(1) : w))
			.join(' ');
		objectStamps.push({ key, group, label, url: toUrl(join(objectsDir, f)) });
	}
} catch {
	/* no Objects/ dir yet */
}
...
// Stamp keys: the basename of every Objects/ PNG. The paint_stroke validator
// checks stamp-mode textureKeys against this set (parity with how terrain keys
// gate fill mode) so a junk stamp key can't be stored. Declaration-only.
const stampKeys = objectStamps.map((o) => o.key).sort();
// Stamp GROUP keys (WM3 Slice C — varied scatter): a stamp-mode textureKey may
// also name a family ("tree_object"); the renderer scatters varied members
// deterministically. Validated alongside the individual keys.
const stampGroupKeys = [...new Set(objectStamps.map((o) => o.group))].sort();
...
const keysTs =
	...
	stampKeys.map((k) => `\t${JSON.stringify(k)}`).join(',\n') +
	'\n] as const;\n\n' +
	'export const STAMP_GROUP_KEYS = [\n' +
	stampGroupKeys.map((k) => `\t${JSON.stringify(k)}`).join(',\n') +
	'\n] as const;\n';
writeFileSync(KEYS_OUT, keysTs);

// Summary to stdout.
const cats = Object.keys(categories);
console.log(`terrain-keys.generated.ts written: ${assetKeys.length} terrain keys + ${stampKeys.length} stamp keys`);
console.log(`terrain-manifest.json written: ${cats.length} land categories + ${waterColors.length} water colors + ${objectStamps.length} stamps`);
for (const c of cats) {
