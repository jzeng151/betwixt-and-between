// Factions — World Map v3 Slice 1b. User-scoped via factions.story_id (no
...
export const GET: RequestHandler = async (event) => {
	...
	return json(await listFactions(db, storyId, { after, limit }));
};
...
export const POST: RequestHandler = async (event) => {
	...
	const created = await createFaction(db, storyId, {
		name: body.name,
		color: body.color,
