...
- **D17 / Issue 11A + 17A — Wrap all interval write paths in `db.transaction(async (tx) => {...})`.** Polymorphic `tx | db` parameter on `actIndexOf`, `sceneIndexOf`, `assertEntityType`, `assertNoOverlap`, `validateFKTypes`, `computeIntervalPositions`, `recomputeIntervalsForAct`, `recomputeAllIntervals`. All callers thread `tx` through. Estimated ~60-100 LOC of mechanical plumbing across `intervals.ts`. Closes the standing PR 1 tech-debt note at `intervals.ts:289`. Pre-empts future Turso replica-lag races.

- **D17-extension** — Sync-mode refactor (deeper PR 1 cleanup converting helper chain to Drizzle's sync mode using better-sqlite3's native sync API) is deferred as a separate follow-up PR (TODO T1).
